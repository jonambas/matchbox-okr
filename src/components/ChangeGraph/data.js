...
const fillByDate = ({ dataSet, fill = {}, from, to } = {}) => {
  const orderedData = dataSet.sort(
    (a, b) => new Date(a.date) - new Date(b.date)
  )
  let filledDataSet = []

  for (
    let time = moment(from), index = 0;
    time.isBefore(moment(to));
    time.add(1, "day")
  ) {
    const dataEntry = orderedData[index]
    const fillData = { ...fill, date: formatInputDate(time) }
    const week =
      moment(fillData.date).year() + "-" + moment(fillData.date).week()
...
function getData() {
  ...
  const formatted = filtered.map(item => {
    const changes = item.changes.reduce(
      (acc, change) => {
        return {
